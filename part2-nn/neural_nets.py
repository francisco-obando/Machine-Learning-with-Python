...
class NeuralNetwork():
    ...
    def predict(self, x1, x2):
        ...
        RELUvectorized = np.vectorize(rectified_linear_unit)
        outputvectorized = np.vectorize(output_layer_activation)

        # Compute output for a single input(should be same as the forward propagation in training)
        hidden_layer_weighted_input = np.dot(self.input_to_hidden_weights,input_values)+self.biases
        hidden_layer_activation = RELUvectorized(hidden_layer_weighted_input)
        output = np.dot(self.hidden_to_output_weights,hidden_layer_activation)
        activated_output = outputvectorized(output)
