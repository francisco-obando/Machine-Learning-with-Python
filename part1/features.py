...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    #Remove feature means d-vector, leaving a nxd matrix
    X_proj = X - feature_means

    #select first n_components of principal components
    # (n x d) x (d x n_componentes) = (n x n_components)
    X_proj = np.dot(X_proj, pcs[:, 0:n_components])
    return X_proj
