...
def bic(X: np.ndarray, mixture: GaussianMixture,
    ...
    # k*d parameters for mu, K-1 for partition, K for variances
    p = K + K + K*d-1
    bic = log_likelihood -0.5*p*np.log(n)
    return float(bic)

def npdf(X:np.ndarray, mu:np.ndarray, covariance:np.ndarray)-> float:
    """
    ...
    """
    d = X.size
    scaling = ((2*np.pi) ** (-d/2)) * (np.linalg.det(covariance) ** (-1/2))
    exparg = (-1/2) * np.transpose((X-mu)) @ (np.linalg.pinv(covariance) @ (X-mu))

    return scaling*np.exp(exparg).flatten()
