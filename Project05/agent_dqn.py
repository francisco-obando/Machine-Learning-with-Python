...
def epsilon_greedy(state_vector, epsilon):
    ...
    """
    # Create population vector with random action choice and outpot of DQN neural network
    action_vec, object_vec = model(state_vector)
    # Validate need for torch.no_grad wrapper
    nn_action_index = torch.argmax(action_vec)
    nn_object_index = torch.argmax(object_vec)
    population = [[np.random.randint(NUM_ACTIONS), np.random.randint(NUM_OBJECTS)], [nn_action_index, nn_object_index]]
    # Select between posible action object
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    # Average of max values of action and objects
    maxq_next = 1 / 2 * (q_values_action_next.max()
                         + q_values_object_next.max())
    # Current state tuple, extract value using given action index and object index
    q_value_cur_state = model(current_state_vector)

    Q = (1/2)*(q_value_cur_state[0][action_index]+q_value_cur_state[1][object_index])

    # Define loss function as (y-q)^2
    # If terminal step dont add maximum value of next step
    Y = reward + GAMMA * maxq_next * (0 if terminal else 1)
    loss = (1/2)*(Y-Q)**2
    # Zero gradients before back propagating
    optimizer.zero_grad()
    # Single pass of back propagation
    loss.backward()
    # Update model parameters with global variable optimizer
    optimizer.step()
...
def run_episode(for_training):
    ...
    while not terminal:
        # Get Tensor for vector representation of BoW of current state description
        current_state = current_room_desc + current_quest_desc
        current_state_vector = torch.FloatTensor(
            utils.extract_bow_feature_vector(current_state, dictionary))
        # Choose action from epsilon greedy algorithm
        new_action_index, new_object_index = epsilon_greedy(current_state_vector, epsilon)
        # Take a step in game
        next_room_desc, next_quest_desc, reward, terminal = framework.step_game(current_room_desc, current_quest_desc, new_action_index, new_object_index)
        ...
        if for_training:
            # update Q-function.
            deep_q_learning(current_state_vector, new_action_index, new_object_index, reward,
                            next_state_vector, terminal)
